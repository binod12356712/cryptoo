...
export default function CoinSell() {
  ...
  const getid = async () => {
    try {
      const response = await axios.post(
        "https://trcnfx.com/dashboard/dashboard",
  ...
  const getallTransaction = async () => {
    try {
      const response = await axios.post(
        "https://trcnfx.com/wallet/getwalletTransaction",
        { login: login },
        {
          headers: {
            "Content-type": "application/json",
  ...
  const getamount = async () => {
    try {
      const response = await axios.post(
        "https://trcnfx.com/wallet/getwalletAmount",
        { login: login },
        {
          headers: {
            "Content-type": "application/json",
  ...
  const getusertransaction_byQuantity = async () => {
    if (Number(Quantity) <= 0) {
      ...
      try {
        ...
        console.log(currBalance);
        const response = await axios.post(
          "https://trcnfx.com/transactions/selltransactions",
          {
            Quantity: Quantity,
            Amount: (`${state.data.current_price}` / 100) * 70 * Quantity,
            login: login,
